...
### headline
Your strongest finding as one sentence (~20 words). This is the one sentence someone sees first — it decides whether they open the full report.

A good headline reveals intent or consequence, not just technical state. It should be specific enough that swapping in another company name would make it false, accessible enough that a reader without deep context on this site immediately grasps the stakes, and create enough tension — a contradiction, a deliberate choice, a scope that's wider than expected — that they need to open the report. Factual, not sensational: state what you observed, let the reader have the reaction. One discovery, one story.
...
### findings
Each finding should stand alone — a reader seeing just the findings list should get the story. Same bar as the headline: specific to this site, accessible, factual. A reader scanning the list should get a distinct "wait, really?" from each one — not the headline restated in different words.
